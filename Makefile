...
venv: # Create virtual environment if it doesn't exist. (activate with: source .trackerenv/bin/activate)
	@test -d .trackerenv || python3 -m venv .trackerenv;
...
help:
	...
	@grep -E '^[a-zA-Z0-9 -]+:.*#'  Makefile | sort | while read -r l; do \
		...
		printf "\033[1;32m%-20s\033[00m : %s\n" $$target "$$desc"; \
	done
